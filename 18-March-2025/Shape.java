...
public interface Shape {
    ...
    /**
     * Method to check if a point will cross a line if the point is casting a line from its position to xMax
     ...
     * @param index is the index of shape where the shape exist in the shpaelist of screen.
     * @return true, if ray casted by a point crosses the Line of a shape, otherwise false
     */
    public default boolean isRayCasting(List<Point> allPointsList, Point pointToCheckObj, int index) {
        ...
        if (pointToCheckObj.yCordinate <= yCordinate2 && pointToCheckObj.yCordinate >= yCordinate1) {
            double ratio = ((pointToCheckObj.yCordinate - yCordinate1) / (yCordinate2 - yCordinate1))*(xCordinate2-xCordinate1);
     ...
     * @param pointObj  is the Point for which user wants to check if its enclosed by a shape
